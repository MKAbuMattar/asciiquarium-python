...
| # | Increment | Status |
|---|---|---|
| 9 | **One formatter, one linter.** `black`, `isort`, and `ruff` are all configured, and `ruff` already does both other jobs (`"I"` is in the selected rule set). The two configs disagree: `isort` sets `line_length = 88`, `ruff` sets `line-length = 100`. Collapsed onto `ruff format` + `ruff check --fix`. | ✅ done — one reformat commit at ruff's already-declared 100 columns, `black` and `isort` gone from every dev list, and `ruff format --check` is now a CI gate. The art was hashed before and after to prove no shape or colour mask moved |
...
| 16 | **Delete what is already dead.** `MANIFEST.in` (hatchling ignores it), the `callback_args == "hooked"` string branch in `fishhook_cb`, the unused `position()` call in `retract()`. The `elif showing_info: pass` no-op went with the frame clock in (2). | planned |
...
| 22 | **Release workflow.** `.github/workflows/release.yml`. Branch as `release/<version>`, open a pull request to `main`, add the `release` label. Preparing (bump, changelog, gate, build, wheel smoke test) happens while the PR is open and is idempotent; **merging** is the single irreversible act that uploads to PyPI, tags, and writes the Release page. Split that way because PyPI forbids re-uploading a version, so the point of no return should be one deliberate click and the PR diff should show exactly what ships. Authenticates with the `PYPI_API_TOKEN` secret on the `pypi` environment. | ✅ done — hardened after 2.4.1 was failed by its own confirmation step: five-minute window, both PyPI views accepted, and the job is re-runnable |
...
| 36 | **Expose the tuning constants as flags.** `--fps`, `--speed`, `--density`, `--no-castle`. Deliberately flags and not the config file rejected below: one `argparse` line and one existing constant made a parameter, with no schema, no search path, and no migration story. `--density` maps onto the existing `screen_size // 350` fish count. `--fps` was blocked on (2) and is not any more: the loop now paces off `FRAME_INTERVAL`, so the flag is that constant made settable rather than a number the loop ignores. | planned |
